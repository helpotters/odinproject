...
# Long methods are like a run-on sentence that should have been divided into 2 or 3 different sentences so that everything
# could be clearly understood and in this case if a method does many different things it can be difficult to test.
...
describe NumberGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        user_input = 'three'
        incorrect_input = game_check.verify_input(user_input)
        expect(incorrect_input).to be_nil
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      ...
      before do
        ...
        letter = 'd'
        valid_input = '8'
        # We are creating a fake object and setting the input and outputs
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_loop).to receive(:player_input).and_return('wrong', 'incorrect', 5)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game) { NumberGame.new(8, 8, 3) }
      # remove the 'x' before running this test
