...
# The file order to complete this lesson:1. Familarize yourself with the class in lib/12_magic_seven.rb 2. Complete spec/12_magic_seven_spec.rb
...
describe MagicSeven do
  ...
  describe '#divide_by_two' do
    context 'the dividend is 10' do
      it 'returns 5' do
        dividend = 10
        quotient = game.divide_by_two(dividend)
  ...
  describe '#subtract_random_number' do
    ...
    subject(:magic_seven) { MagicSeven.new(random_number) }

    context "when we have random number #{random_number}" do
      it "will return #{random_number - 7}" do
        number = 7
        expect(magic_seven.subtract_random_number(number)).to eq(number - random_number)
      end
    end
  end
