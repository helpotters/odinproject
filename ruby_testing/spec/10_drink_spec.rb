...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      subject(:matcha_green_tea) { described_class.new('matcha green tea') }
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(matcha_green_tea.type).to eq('matcha green tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(matcha_green_tea.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      subject(:black_tea) { described_class.new('black tea') }
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.

      # remove the 'x' before running this test
      it 'is full' do
        expect(black_tea).to be_full
      end
    end

    context 'when drink has less than 16 ounces' do
      subject(:sporkle) { described_class.new('sporkle', 1) }
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type. In addition, specify ounces to be any number under 16.

      # remove the 'x' before running this test
      it 'is not full' do
        expect(sporkle).not_to be_full
      end
    end
  end
end
